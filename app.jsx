...
function ListPage(){
  ...
  return (
    <div className="wrap">
      ...
      <div className="grid">
        {(loading ? Array.from({length:12}) : data.recipes).map((r,i)=>(
          <Link className="card" key={r?.id ?? i} to={`/recipe/${r?.id ?? 0}`}>
            ...
            <div className="card-body">
              ...
              {!loading && (
                <div className="chips">
                  {(r.tags ?? r.mealType ?? []).slice(0,3).map(t=><span className="chip" key={t}>{t}</span>)}
                </div>
              )}
            </div>
          </Link>
...
function DetailPage(){
  ...
  if(loading) return (<div className="wrap"><header>천개의 레시피</header><div style={{padding:16}}>Loading…</div></div>);
  if(err) return (<div className="wrap"><header>천개의 레시피</header><div className="err">{err}</div></div>);
      ...
      <div className="row" style={{marginTop:16}}>
        ...
        <div>
          ...
          <div style={{marginTop:8,display:"flex",gap:6}}>
            {(r.tags ?? r.mealType ?? []).slice(0,3).map(t=>(<span key={t} className="chip">{t}</span>))}
          </div>
        </div>
      </div>
